...
# Test function for validation (defined after minification functions)
...
function Start-Processing {
    ...
    try {
        ...
        $fileResult = Get-FilesFromPaths -Paths $global:fileList -IncludeFilter $includeFilter -ExcludeFilter $excludeFilter
        $filesToProcess = $fileResult.Files

        # Build status message with file counts
        $statusMessage = "Found $($fileResult.FinalCount) files to process"
        if ($fileResult.ExcludedCount -gt 0) {
            $statusMessage += " ($($fileResult.ExcludedCount) files excluded by filters)"
        }

        if ($filesToProcess.Count -eq 0) {
            # Show a more informative message based on what happened
            if ($fileResult.OriginalCount -eq 0) {
                throw "No files found in the selected paths."
            } elseif ($fileResult.AfterIncludeCount -eq 0 -and (-not [string]::IsNullOrWhiteSpace($includeFilter))) {
                throw "No files match the include filter '$includeFilter'. Check your include extensions."
            } elseif ($fileResult.ExcludedCount -gt 0) {
                throw "All $($fileResult.AfterIncludeCount) matching files were excluded by the exclude filter '$excludeFilter'."
            } else {
                throw "No files found to process. Check your include/exclude filters."
            }
        }

        # Show warning if files were excluded
        if ($fileResult.ExcludedCount -gt 0) {
            $warningMsg = "Warning: $($fileResult.ExcludedCount) files were excluded by filters and will not be processed."
        ...
        $completionMsg = "Context generation completed successfully!`n`nProcessed $($filesToProcess.Count) files"
        if ($fileResult.ExcludedCount -gt 0) {
            $completionMsg += " ($($fileResult.ExcludedCount) files were excluded by filters)"
        }
        $completionMsg += "`n`nOutput saved to: $outputPath"

        # Show completion message but don't close the application
        [System.Windows.Forms.MessageBox]::Show($completionMsg, "Success", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Information)
...
try {
    ...
    $includeLabel.Text = "Include Extensions (comma-separated):"
    ...
    $includeExtBox.Text = "*.cs, *.js, *.py, *.ts, *.tsx, *.jsx, *.mdx"

    # Exclude extensions
    $excludeLabel = New-Object System.Windows.Forms.Label
    $excludeLabel.Text = "Exclude Extensions (comma-separated):"
    ...
    $excludeExtBox.Text = "*.exe, *.dll, *.bin, *.svg, *.ico"
